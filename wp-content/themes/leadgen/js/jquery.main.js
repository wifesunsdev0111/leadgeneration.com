...
function initCarousel() {
	jQuery('div.logos-list-2').scrollGallery({
		mask: 'div.mask',
		slider: 'div.slideset',
		slides: 'div.slide',
		btnPrev: 'a.btn-prev',
	...
	jQuery('div.mini-gallery').scrollGallery({
		mask: 'div.mask',
		slider: 'div.slideset',
		slides: 'div.slide',
		btnPrev: 'a.btn-prev',
	...
	jQuery('.logo-gallery').scrollGallery({
		mask: '.gmask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: '.btn-prev',
...
function initCycleCarousel() {
	jQuery('div.logos-list').scrollAbsoluteGallery({
		mask: 'div.mask',
		slider: 'div.slideset',
		slides: 'div.slide',
		btnPrev: 'a.btn-prev',
	...
	jQuery('div.logos-list-2').scrollAbsoluteGallery({
		mask: 'div.mask',
		slider: 'div.slideset',
		slides: 'div.slide',
		btnPrev: 'a.btn-prev',
...
function initSlideShow() {
	jQuery('div.quote-slide').fadeGallery({
		slides: 'div.slide',
		btnPrev: 'a.btn-prev',
		btnNext: 'a.btn-next',
		event: 'click',
...
function initOpenClose() {
	jQuery('.solutions-block').openClose({
		activeClass: 'active',
		opener: '.opener',
		slider: '.slide-block',
		animSpeed: 400,
...
function initSameHeight() {
	jQuery('div.blocks-holder').sameHeight({
		elements: 'div.solutions-block',
		flexible: true,
		multiLine: true
	});
}
...
function initTouchNav() {
	...
	ResponsiveHelper.addRange({
		'988..': {
			...
			off: function() {
				jQuery('#nav').replaceWith(nav.clone());
			}
		}
	});
...
function initSlidePanel(){
	...
	var btnTop = jQuery('#back-top-wrapper');
	var wrapper = jQuery('body');
	var panel = wrapper.find('.left-side');
	var content = wrapper.find('#wrapper');
	var opener = panel.find('.open-left-side');
	var close = panel.find('.close');
	var slide = panel.find('.panel');
	var addBlock = jQuery('.social-side');
	var top = 0;
	var animated = false;
	...
	var closeHandler = function (){
		if (wrapper.hasClass(activeClass) && !animated) {
			hideSlide();
		}
	}
	var showSlide = function(){
		animated = true;
		btnTop.stop().animate({marginLeft: panel.outerWidth()},{queue: false, duration:animSpeed});
		opener.stop().animate({left: panel.outerWidth()},{queue: false, duration:animSpeed, complete: function(){
			opener.fadeOut(animSpeed, function(){
				animated = false;
			});
		}});
		addBlock.stop().animate({left: panel.outerWidth()},{queue: false, duration:animSpeed});
		panel.stop().animate({marginLeft: 0},{queue: false, duration:animSpeed});
	}
	var hideSlide = function(){
		animated = true;
		btnTop.stop().animate({marginLeft: 0},{queue: false, duration:animSpeed});
		panel.stop().animate({marginLeft: -panel.outerWidth()},{queue: false, duration:animSpeed, complete: function(){
			opener.css({left:''}).fadeIn(animSpeed, function(){
				animated = false;
			});
			wrapper.removeClass(activeClass);
		}});
		addBlock.stop().animate({left: 0},{queue: false, duration:animSpeed, complete: function(){
			addBlock.css({left:''});
		}});
	}
	...
	ResponsiveHelper.addRange({
		'768..': {
			on: function() {
				...
				content.on('click', closeHandler);
			},
			off: function() {
				close.off('click', closeHandler);
				opener.off('click', clickHandler);
				content.off('click', closeHandler);
				wrapper.removeClass(activeClass);
				panel.stop().css({marginLeft:''});
				addBlock.removeAttr('style');
				opener.stop().css({left:'', display:''});
				btnTop.css({marginRight:''});
			}
		}
	});
}
...
var ImageStretcher = {
	...
	add: function(options) {
		...
		if(!this.win) {
			...
			clearTimeout(this.timer);
			this.timer = setTimeout(self.resizeHandler, 100);
		}
